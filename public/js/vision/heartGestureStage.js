...
// particle size for phone screen 
 ...
 * (.particle / .particle-heart classes + sparkleRise / sparkleRiseHeart
 * keyframes from playlist.css, loaded globally) — same mixed round/heart
 * shapes, rising from the bottom of the screen and fading, as seen when
 * a song plays. This is the SAME mechanism as libraryCompletionSparkle.js
 * on the Library page — both create their own overlay and append it
 * directly to <body>, rather than depending on any static element in
 * index.html.
 *
 * Particle sizes here are intentionally LARGER than Playlist's — this
 * burst covers the full viewport (not a small card area), and the
 * original Playlist-tuned sizes (4-15px dots, 12-26px hearts) read as
 * near-invisible specks on a full-screen mobile burst.
 *
 * IMPORTANT — two separate bugs, both now avoided:
 *  1. playlist.css scopes .particle/.particle-heart styling and the
 *     rise animations to ".playlist-particles-container .particle[-heart]"
 ...
 * so no changes to app.js are needed.
 *
 * NOTE: the Continue button's second click calls App.navigateTo("contact").
 * "App" isn't imported anywhere in this file — if it isn't a global
 * exposed elsewhere (e.g. window.App set in app.js), that click will throw
 * a ReferenceError and silently fail to navigate. Confirm App is a global
 * before relying on this, or wire in the correct navigation call/import.
 */
import visionEngine from "./visionEngine.js";
import { initAirWriting } from "./airWriting.js";
import { initOutfitColorLabel, showOutfitColorPopup } from "./outfitColorLabel.js";
...
export function initHeartGestureStage({
  ...
  function spawnParticle(zone) {
    ...
    if (isHeart) {
      particle.classList.add("particle-heart");
      particle.textContent = "♥";
      const size = 16 + Math.random() * 16; // 16–32px (bumped up from 12–26px for full-viewport visibility)
      particle.style.fontSize = `${size}px`;
    } else {
      particle.classList.add("particle");
      const sizeRoll = particleCounter % 3;
      const size = sizeRoll === 0 ? 6 + Math.random() * 3    // small: 6–9px
                 : sizeRoll === 1 ? 10 + Math.random() * 4   // medium: 10–14px
                 : 15 + Math.random() * 6;                   // big: 15–21px
      particle.style.width = `${size}px`;
      particle.style.height = `${size}px`;
    }
