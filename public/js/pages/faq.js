...
const FaqPage = (() => {
  ...
  /**
   * Create a falling sparkle celebration — a mix of round dots,
   * hearts, and stars in random sizes, instead of emoji stickers.
   */
  function createCelebration() {
    const container = document.createElement("div");
    container.classList.add("celebration-hearts");
    ...
    const shapeTypes = ["dot", "heart", "star"];

    for (let i = 0; i < 40; i++) {
      const type = shapeTypes[i % shapeTypes.length];
      const particle = document.createElement(type === "dot" ? "div" : "span");
      particle.classList.add("celebration-particle", `celebration-${type}`);
      ...
      if (type === "heart") {
        ...
        const size = 6 + Math.random() * 12; // 6px–18px
