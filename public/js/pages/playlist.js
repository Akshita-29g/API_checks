...
const PlaylistPage = (() => {
  ...
  function init(contentData) {
    ...
    playerBar = document.getElementById("spotify-player-bar");
    embedContainer = document.getElementById("spotify-embed-container");
    particlesContainer = document.getElementById("playlist-particles-container");
  ...
  function buildSongCards(songs) {
    ...
    songList.forEach((song) => {
      ...
      if (trackId) {
        card.addEventListener("click", () => openTrack(trackId));
        card.addEventListener("keydown", (e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            openTrack(trackId);
  ...
  /**
   * Open the embedded player and play the track normally,
   * starting from the beginning — no seeking to any timestamp.
   */
  function openTrack(trackId) {
    ...
    if (!spotifyApi) {
      // API script hasn't finished loading yet; try again shortly.
      setTimeout(() => openTrack(trackId), 300);
      return;
    }
